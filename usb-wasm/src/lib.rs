wasmtime::component::bindgen!({
    ...
    with: {
        ...
        "component:usb/device/usb-configuration": UsbConfiguration,
        "component:usb/device/usb-interface": UsbInterface,
        "component:usb/device/usb-endpoint": UsbEndpoint,
    }
});

use error::UsbWasmError;
use rusb::{constants::LIBUSB_TRANSFER_TYPE_ISOCHRONOUS, ffi::{libusb_alloc_transfer, libusb_handle_events_completed, libusb_submit_transfer}, Context, Device, GlobalContext, Hotplug, Recipient, RequestType, Speed, UsbContext};
use std::{error::Error, sync::Arc, time::Duration};
use std::process::exit;
use tokio::sync::mpsc;
use component::usb::{self, types::Direction};
...
impl DeviceUpdateHandler {
    fn new(buffer_size: usize) -> Result<(mpsc::Receiver<DeviceConnectionEvent>, rusb::Registration<rusb::Context>, std::thread::JoinHandle<()>)> {
...
impl DeviceUpdateHandler {
    fn start_listener(self) -> Result<(rusb::Registration<rusb::GlobalContext>, std::thread::JoinHandle<()>)> {
        let context = rusb::Context::new().unwrap();
        let reg: rusb::Registration<rusb::GlobalContext> = rusb::HotplugBuilder::new()
            .enumerate(true)
...
impl Hotplug<rusb::GlobalContext> for DeviceUpdateHandler {
    fn device_arrived(&mut self, device: rusb::Device<rusb::GlobalContext>) {
        let sender = self.sender.clone();
        let mydevice = read_device(device).unwrap();
        let _ = sender.send(DeviceConnectionEvent::Connected(mydevice));
    }

    fn device_left(&mut self, device: rusb::Device<rusb::GlobalContext>) {
        let sender = self.sender.clone();
        let mydevice = read_device(device).unwrap();
        let _ = sender.send(DeviceConnectionEvent::Disconnected(mydevice));
    }
}

pub fn device_connection_updates() -> Result<(mpsc::Receiver<DeviceConnectionEvent>, rusb::Registration<rusb::Context>, std::thread::JoinHandle<()>)> {
